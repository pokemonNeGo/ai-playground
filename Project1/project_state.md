...
| 2.1|Chunking: фиксированный размер|✅ Выполнена|2026-09-17|chunk_size=500 символов, overlap=50 символов (шаг 450); 138 785 чанков из 4910 документов; таблица  chunks  в SQLite|
...
Лог Лаб 2.1: `data/processed/lab2_1_chunking.log`
...
Лаб 2.1 — `scripts/lab2_1_chunk_corpus.py`.
...
#### Стратегия A (фиксированный размер) — Лаб 2.1 ✅
Размер чанка: 500 символов
Перекрытие (overlap): 50 символов
Шаг между началами чанков: chunk_size − overlap = 450 символов
Единица измерения: символы. Решение: для первой итерации выбрано простое «скользящее окно» по символам; переход к токенам возможен в Лаб 2.3 при необходимости.
Формат chunk_id: `{doc_id}_c{chunk_index:04d}` (например, `7_c0000`)
Точка входа: `chunk_text_fixed_size()` в `src/chunking/fixed_size.py`
Скрипт обработки: `scripts/lab2_1_chunk_corpus.py`
Скрипт проверки: `scripts/lab2_1_check_db.py`
...
ai-playground/
 ...
 └── Project1/                   # Основной проект 1
     ...
     │   ├── lab2_1_chunk_corpus.py                   # Лаб 2.1 ✅
     │   └── lab2_1_check_db.py                       # Лаб 2.1 ✅
     │
     ├── tests/                  # хранится в Git
     │   ├── test_parser.py                           # Лаб 1.2 ✅ (8 тестов)
     │   ├── test_normalize.py                        # Лаб 1.3 ✅ (6 тестов)
     │   └── test_chunking.py                         # Лаб 2.1 ✅ (4 теста)
     │
...
pathlib
datetime
Для Лаб 2.1 новые внешние зависимости не добавлялись.
Использованы стандартные модули:
sqlite3
json
logging
...
tqdm: [версия] — прогресс-бары (использовалась в Лаб 1.1 и Лаб 2.1)
...
Реализовано в Лаб 2.1 (Стратегия A): чанки хранятся в таблице `chunks` в той же SQLite-базе, что и документы.
SQL-схема:
CREATE TABLE chunks (
...
Чанкинг Лаб 2.1 выполнен в символах, а не в токенах: границы чанков не совпадают с границами токенов эмбеддинг-модели. При необходимости переход на токен-окна планируется в Лаб 2.3.
Фиксированный размер чанка может разрывать предложения и смысловые блоки на границах; перекрытие 50 символов частично компенсирует это. Альтернатива — структурное чанкинг (Лаб 2.2).
...
Хранение `start_char`/`end_char` в чанке позволяет детерминированно проверять перекрытие и позже восстанавливать точную позицию найденного фрагмента в исходном документе (пригодится для цитат в Лаб 3.4).
...
Лаб 2.1 выполнена: чанкинг фиксированного размера с перекрытием реализован.
...
Скрипт Лаб 2.1: `scripts/lab2_1_chunk_corpus.py`; проверка: `scripts/lab2_1_check_db.py` (все проверки зелёные).
Тесты Лаб 2.1: `tests/test_chunking.py` зелёные (4 теста).
Решение по хранению статистик в Git: оставляем `data/` полностью игнорируемой; все артефакты воспроизводимы скриптами.
Готово к Лаб 2.2 (структурное чанкинг) или Лаб 2.3 (эмбеддинги и Qdrant).
...
### Быстрый перезапуск Лаб 2.1
Из папки `Project1` с активированным виртуальным окружением:
python -m pytest tests/test_chunking.py -q
python -m scripts.lab2_1_chunk_corpus
python scripts/lab2_1_check_db.py
Примечание: если при запуске скрипта возникает `ModuleNotFoundError: No module named 'src'`, запускать именно как модуль (`python -m scripts.lab2_1_chunk_corpus`) либо добавить корень проекта в `sys.path` внутри скрипта.
Ожидаемый результат:
